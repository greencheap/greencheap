    ...
    <div>
                ...
                <div class="uk-margin">
                    ...
                    <div class="uk-form-controls uk-form-controls-text">
                        <p class="uk-margin-small">
                            <label
                                ><input v-model="widget.content" class="uk-radio" type="radio" value="" /><span class="uk-margin-small-left">{{ "Don't show" | trans }}</span></label
                            >
                        </p>
                        <p class="uk-margin-small">
                            <label
                                ><input v-model="widget.content" class="uk-radio" type="radio" value="1" /><span class="uk-margin-small-left">{{ "Show on all posts" | trans }}</span></label
                            >
                        </p>
                        <p class="uk-margin-small">
                            <label
                                ><input v-model="widget.content" class="uk-radio" type="radio" value="2" /><span class="uk-margin-small-left">{{ "Only show on first post." | trans }}</span></label
                            >
                        </p>
                    </div>
                </div>
        ...
        <div class="uk-card-body">
            <div v-if="status != 'loading'">
                <h3 v-if="widget.title" class="uk-h5"><i uk-icon="calendar" class="uk-margin-small-right"></i>{{ widget.title }}</h3>
                <div v-else class="uk-margin-bottom">
                    <h3 v-if="!widget.title && head.title" class="uk-h5 uk-margin-remove-bottom"><i uk-icon="calendar" class="uk-margin-small-right"></i>{{ head.title }}</h3>
                ...
                <ul class="uk-list uk-list-large uk-list-divider uk-margin-remove">
                    <li v-for="(entry, key) in count" :key="key">
                        <a v-if="entry.thumbnail" :href="entry.link" target="_blank"><img :data-src="entry.thumbnail" class="tm-feed-image" uk-img /></a>
                        <a class="uk-link-text uk-h5 uk-text-bold" :href="entry.link" target="_blank" v-html="entry.title" />
...
export default {
    ...
    methods: {
        load() {
            ...
            // TODO: Add cache
            // rss2json
            this.$http.get(`https://api.rss2json.com/v1/api.json?rss_url=${this.widget.url}`, { params: { cache: 60 } }).then(
                function (res) {
                    const { data } = res;
