...
class CaptchaListener implements EventSubscriberInterface
{
    ...
    {
        if (!App::module("system/captcha")->config("recaptcha_enable") || App::user()->isAuthenticated() || !($routes = App::request()->attributes->get("_captcha_routes")) || !($sitekey = App::module("system/captcha")->config("recaptcha_sitekey"))) {
            return;
        }
    ...
    public function onRequest($event, $request)
    {
        ...
        if (!App::module("system/captcha")->config("recaptcha_enable") || !($captcha = $request->attributes->get("_captcha_verify")) || App::user()->isAuthenticated()) {
            return;
        }
    ...
    protected function verifyToken($gRecaptchaResponse, $secret)
    {
        ...
        if ($gRecaptchaResponse && $secret) {
            $result = json_decode(
                $this->post("https://www.google.com/recaptcha/api/siteverify", [
                    "secret" => $secret,
