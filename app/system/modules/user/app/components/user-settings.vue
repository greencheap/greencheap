    ...
    <div uk-grid class="uk-grid-small">
        <div class="uk-width-2-3@s">
            <div class="uk-margin">
                ...
                <v-input id="form-username" name="username" type="text" view="class: uk-form-width-large uk-input" v-model="user.username" :rules="{ required: true, regex: /^[a-zA-Z0-9._\-]+$/ }" autocomplete="new-username" message='Username cannot be blank and may only contain alphanumeric characters (A-Z, 0-9) and some special characters ("._-")' />
            </div>

            <div class="uk-margin">
                <label for="form-name" class="uk-form-label">{{ "Name" | trans }}</label>
                <v-input id="form-name" name="name" type="text" view="class: uk-form-width-large uk-input" v-model="user.name" :rules="{ required: true }" autocomplete="new-name" message="Name cannot be blank." />
            </div>

            <div class="uk-margin">
                <label for="form-email" class="uk-form-label">{{ "Email" | trans }}</label>
                <v-input id="form-email" name="email" type="email" view="class: uk-form-width-large uk-input" v-model.lazy="user.email" :rules="{ required: true, email: true }" autocomplete="new-email" message="Field must be a valid email address." />
            </div>
            ...
            <div class="uk-margin">
                ...
                <div class="uk-form-controls uk-form-controls-text">
                    <p v-for="(status, key) in config.statuses" :key="key" class="uk-margin-small">
                        <label
                            ><input v-model="user.status" class="uk-radio" type="radio" :value="parseInt(key)" :disabled="config.currentUser == user.id" /><span class="uk-margin-small-left">{{ status }}</span></label
                        >
                    </p>
                </div>
            </div>
            ...
            <div class="uk-margin">
                ...
                <div class="uk-form-controls uk-form-controls-text">
                    <p v-for="role in config.roles" :key="role.id" class="uk-margin-small">
                        <label
                            ><input v-model="user.roles" class="uk-checkbox" type="checkbox" :value="role.id" :disabled="role.disabled" /><span class="uk-margin-small-left">{{ role.name }}</span></label
                        >
                    </p>
                </div>
            </div>
                ...
                <div class="uk-width-5-6@s">
                    <div v-show="user.name" class="uk-card uk-card-default uk-text-center">
                        ...
                        <div class="uk-card-footer">
                            ...
                            <div>
                                <a class="uk-text-break uk-text-small" :href="'mailto:' + user.email">{{ user.email }}</a
                                ><i v-show="config.emailVerification && user.data.verified" uk-icon="icon: check" :title="'Verified email address' | trans" />
                            </div>
                        </div>
                    </div>
                </div>
