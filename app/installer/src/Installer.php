...
class Installer
{
    ...
    /**
     * @var Application GreenCheap Application instance
     */
    protected $app;
            ...
            try {
                ...
                if (
                    $this->app
                        ->db()
                        ->getUtility()
                        ->tableExists("@system_config")
                ) {
                    $status = "tables-exist";
                    $message = __("Existing GreenCheap installation detected. Choose different table prefix?");
                } else {
                    $status = "no-tables";
                }
    ...
    public function install($config = [], $option = [], $user = [])
    {
        ...
        try {
            ...
            foreach (glob($this->app->get("path.packages") . "/*/*/composer.json") as $package) {
                ...
                if ($package->get("type") === "greencheap-extension" || $package->get("type") === "greencheap-theme") {
