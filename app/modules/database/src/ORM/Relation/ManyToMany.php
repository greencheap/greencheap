...
namespace GreenCheap\Database\ORM\Relation;

use GreenCheap\Database\ORM\QueryBuilder;
...
class ManyToMany extends Relation
{
    ...
    public function resolve(array $entities, QueryBuilder $query)
    {
        ...
        $targets = $query
            ->whereIn($this->keyTo, function ($query) use ($keys, $table, $to, $from) {
                return $query
                    ->select($to)
                    ->from($table)
                    ->whereIn($from, $keys);
        ...
        foreach ($mapping as $id => $targetIds) {
            $entity = current(
                array_filter($entities, function ($entity) use ($metadata, $from, $id) {
            ...
            $metadata->setValue(
                $entity,
                $this->name,
                array_filter($targets, function ($target) use ($targetMetadata, $to, $targetIds) {
